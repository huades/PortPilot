...
function renderProfiles() {
    ...
    const filtered = query ? state.profiles.filter(profile => [profile.name, profile.host, profile.scheme, ...(profile.tags || [])].some(value => String(value).toLowerCase().includes(query))) : state.profiles;
...
function renderLanguage() { document.documentElement.lang = state.language === "zh" ? "zh-CN" : "en"; document.querySelectorAll("[data-i18n]").forEach(el => { el.textContent = t(el.dataset.i18n); }); document.querySelectorAll("[data-i18n-placeholder]").forEach(el => { el.placeholder = t(el.dataset.i18nPlaceholder); }); $("#language").textContent = state.language === "zh" ? "EN" : "中文"; renderPresets(); renderTheme(); renderProfiles(); renderUaAction(); renderAutoDisconnect(); if (state.editingId) $("#saveProfile").textContent = t("save"); if (!$("#checkResult").dataset.message)
...
function renderAutoDisconnect() { const button = $("#autoDisconnect"); button.textContent = t(autoDisconnectEnabled ? "autoDisconnectOn" : "autoDisconnectOff"); button.classList.toggle("off", !autoDisconnectEnabled); }
...
async function init() { const saved = await chrome.storage.local.get(["profiles", "activeProfileId", "lastProfileId", "userAgent", "language", "theme", "autoDisconnect", "autoDisconnectEvent"]); state = { profiles: saved.profiles || [], activeProfileId: saved.activeProfileId || null, lastProfileId: saved.lastProfileId || saved.activeProfileId || null, userAgent: saved.userAgent || null, language: saved.language || "zh", theme: saved.theme || "system", listCollapsed: false, searchQuery: "", editingId: null, testResults: new Map() }; autoDisconnectEnabled = saved.autoDisconnect !== false; $("#ua").value = state.userAgent || ""; renderLanguage(); renderAutoDisconnect(); if (saved.autoDisconnectEvent) { showMessage("autoDisconnected"); await chrome.storage.local.remove("autoDisconnectEvent"); } }
...
$("#autoDisconnect").addEventListener("click", async () => { const next = !autoDisconnectEnabled; const result = await chrome.runtime.sendMessage({ type: "SET_AUTO_DISCONNECT", value: next }); if (!result?.ok) { showMessage("failed"); return; } autoDisconnectEnabled = next; renderAutoDisconnect(); });
...
chrome.storage.onChanged.addListener((changes, areaName) => { if (areaName !== "local") return; if (changes.activeProfileId) { state.activeProfileId = changes.activeProfileId.newValue || null; renderProfiles(); } if (changes.autoDisconnectEvent?.newValue) { showMessage("autoDisconnected"); void chrome.storage.local.remove("autoDisconnectEvent"); } });
void init();
export {};
