...
async function init() { const saved = await chrome.storage.local.get(["profiles", "activeProfileId", "lastProfileId", "userAgent", "language", "theme", "autoDisconnect", "autoDisconnectEvent"]); state = { profiles: saved.profiles || [], activeProfileId: saved.activeProfileId || null, lastProfileId: saved.lastProfileId || saved.activeProfileId || null, userAgent: saved.userAgent || null, language: saved.language || "zh", theme: saved.theme || "system", listCollapsed: false, searchQuery: "", editingId: null, testResults: new Map() }; $("#ua").value = state.userAgent || ""; $("#autoDisconnect").checked = Boolean(saved.autoDisconnect); renderLanguage(); if (saved.autoDisconnectEvent) { showMessage("autoDisconnected"); await chrome.storage.local.remove("autoDisconnectEvent"); } }
...
$("#autoDisconnect").addEventListener("change", async event => { const result = await chrome.runtime.sendMessage({ type: "SET_AUTO_DISCONNECT", value: event.target.checked }); if (!result?.ok) { event.target.checked = !event.target.checked; showMessage("failed"); } });
