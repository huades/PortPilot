...
- **手动折叠：**需要精简界面时点击“折叠列表”，折叠状态只显示当前节点；点击“展开列表”恢复全部节点。
- **启用代理：**点击对应代理卡片，当前节点会保持高亮，列表不会自动折叠。
- **关闭代理：**点击“关闭代理”后，Chrome 不再使用 PortPilot 代理，恢复通过本机网络直接访问网站。
- **断线保护：**默认开启，可点击“断线保护：开启/关闭”按钮切换。后台每分钟检测两个独立网络端点，连续三轮均失败后关闭代理并切回本机网络，以减少短暂超时造成的误判。
